...
impl CompressedEdwardsY {
    pub fn decompose_with_hints(
        ...
        let carry = add::<8, 1>(&mut rhs, &[1u32]);
        assert_eq!(carry, 0);
        let carry = add::<8, 8>(&mut rhs, &x_square);
        assert_eq!(carry, 0);
...
impl TEPoint {
    pub fn check_add_hint(&self, rhs: &Self, hint: &Self) -> Result<(), EvaluationError> {
        ...
        let carry = add::<8, 8>(&mut should_be_y3, &y1y2);
        assert_eq!(carry, 0);
        should_be_y3 = mul_mod(&should_be_y3, &ONE, &MODULUS_Q);
        let carry = add::<8, 8>(&mut should_be_y3, &x1x2);
        assert_eq!(carry, 0);
        should_be_y3 = mul_mod(&should_be_y3, &ONE, &MODULUS_Q);
    ...
    pub fn check_dbl_hint(&self, hint: &Self) -> Result<(), EvaluationError> {
        ...
        let carry = add::<8, 1>(&mut dx1x1y1y1_plus_one, &[1u32]);
        assert_eq!(carry, 0);
        ...
        let carry = add::<8, 8>(&mut should_be_y3, &y1y1);
        assert_eq!(carry, 0);
        should_be_y3 = mul_mod(&should_be_y3, &ONE, &MODULUS_Q);
        let carry = add::<8, 8>(&mut should_be_y3, &x1x1);
        assert_eq!(carry, 0);
        should_be_y3 = mul_mod(&should_be_y3, &ONE, &MODULUS_Q);
